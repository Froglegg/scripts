...
var sortedArray = bubble_sort(unsortedArr);
// bubble sort function takes in one argument, the unsorted array, then returns a sorted array
function bubble_sort(arr) {
    //begins with a simple loop over the unsorted array
    for (i = 0; i < arr.length; i++) {
        //implements a nested for loop, which iterates over each iteration... J is equal to the array item before... ???
        for (j = arr.length - 1; j > i; j--) {
            //if the array item before is greater than j, move that item up an index number???
            if (arr[j] < arr[j - 1]) {
